...
* Create two postgres databases: `podcast-project` and `podcast-project-test` (you can substitute these with the name of your own application - just be sure to go through and change the `package.json` and `server/db/db.js` to refer to the new names)
  * By default, running `npm test` will use `podcast-project-test`, while regular development uses `podcast-project`
