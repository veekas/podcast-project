...
const ezraMeta = require('../data/ezra-klein-show.json')
const ezraEpisodes = require('../data/ezra-klein-show-episodes.json')
...
db.sync({ force: true })
  .then(function () {
    console.log('Dropped old podcast data, now inserting new podcasts');
    return Promise.map(ezraMeta, function (podcast) {
      return Podcast.create(podcast);
    })
  })
  .then(function () {
    console.log('Dropped old episode data, now inserting new episodes');
    return Promise.map(ezraEpisodes, function (episodes) {
  ...
  .then(function () { //write finally db.close
    console.log('Finished inserting stuff (press ctrl-c to exit)');
  })
  .catch(function (err) {
